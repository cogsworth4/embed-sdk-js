...
const getMainElement = ({ title, text, }) => {
  ...
  `;
    const style = 'padding: 2rem; border: 1px solid #e5e7eb; background: #fff; text-align: center;';
...
const getButton = ({ label, onClick, link, }) => {
    ...
    const style = 'display: inline-block; background-color: #7fcacb; color: #fff; font-weight: 500; border: none; padding: 0.5em 1.5em; border-radius: 5px; margin-top: 1em;';
    element.setAttribute('style', style);
    if (link) {
